...
namespace MiPaladar.Services
{
    ...
    public class TransactionService : ITransactionService
    {
        ...
        {
            UndoSell(prod, quantity, appvm.UnitMeasureManager.Unit, cost, workingDate, 0);
        }
        private void UndoSell(Product prod, double quantity, UnitMeasure um, decimal cost, DateTime workingDate, int level)
        {
            if (prod == null) throw new ArgumentException("Product can't be null", "prod");
            ...
            if (prod.IsRecipe && level == 0)
            {
                foreach (var ing in prod.Ingredients)
                {
                    ...
                    UndoSell(ing_prod, ing_qtty, ing_um, ing_cost, workingDate, level + 1);
                }
            }
            //+quantity
        ...
        void RefreshRecipesCost(Product product)
        {
            foreach (var recipe_item in product.Outgredients)
            {
                ...
                if (!recipe_product.IsRecipe) continue;
